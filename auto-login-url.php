...
/**
 * Generate a auto login URL for any user.
 * this plugin forked one-time-login version 0.3.0.
 *
 * ## OPTIONS
 *
 * <user>
 ...
 * ## EXAMPLES
 *
 *     # Generate two auto login URLs.
 *     $ wp user auto-login-url testuser --count=2
 *     http://wpdev.test/wp-login.php?user_id=2&auto_login_url_token=ebe62e3
 *     http://wpdev.test/wp-login.php?user_id=2&auto_login_url_token=eb41c77
 *
 *     $ wp user auto-login-url testuser --from-date="2019-01-01T00:00:00"
 *     # this url is invalid by from-date, after from-date you can access this url. 
 *
 *     $ wp user auto-login-url testuser --expire-date="2019-03-03" 
 *     # token is expired at "2019-03-03" ,you can access many times up to "2019-03-03"
 *
 *     $ wp user auto-login-url testuser --redirect=/?page_id=8
 */
function auto_login_url_wp_cli_command( $args, $assoc_args ) {
...
//		wp_schedule_single_event( strtotime($expire_date) , 'auto_login_url_cleanup_expired_tokens', array( $user->ID, $tokens ) );
	}

//	if ( $delay_delete ) {
//		$tokens = get_user_meta( $user->ID, 'auto_login_token', true );
//		$tokens = is_string( $tokens ) ? array( $tokens ) : $tokens;
//		wp_schedule_single_event( time() + ( 15 * MINUTE_IN_SECONDS ), 'auto_login_cleanup_expired_tokens', array( $user->ID, $tokens ) );
//	}
	...
	do_action( 'auto_login_url_created', $user );
	foreach ( $new_tokens as $token ) {
		$query_args = array(
			'user_id'              => $user->ID,
			'auto_login_url_token' => $token,
		);
...
if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'user auto-login-url', 'auto_login_url_wp_cli_command' );
}
...
add_action( 'auto_login_url_cleanup_expired_tokens', 'auto_login_url_cleanup_expired_tokens', 10, 2 );
...
add_action( 'init', 'auto_login_url_handle_token' );



// load administrator view
require_once(__DIR__ . '/auto-login-url-admin.php');
